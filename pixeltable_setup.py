...
def _upsert_entry(kb, entry: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        existing_matches = (
            kb.where(kb.path == path)
            .select(kb.type, kb.path, kb.content, kb.title, kb.created_at, kb.metadata)
            .collect()
        )
        ...
        for row in existing_matches:
            ...
            row_service = (
                row_meta.get("service", "unknown") if isinstance(row_meta, dict) else "unknown"
            )
...
def search_knowledge(kb, query: str, type_filter: Optional[str] = None, limit: int = 5):
    ...
    matches = (
        ...
        .select(kb.type, kb.path, kb.title, kb.summary, kb.metadata, kb.is_adr, score=sim)
        .limit(limit)
    )
...
def get_adrs(kb, topic: Optional[str] = None):
    ...
    return list(adrs.select(kb.path, kb.title, kb.summary, kb.created_at).limit(10).collect())


def snapshot_knowledge_base(name: str, tags: List[str] = None):
    """Create a snapshot of the knowledge base"""

    pxt.snapshot("org_knowledge", name=name, tags=tags or [])
...
def delete_service_data(kb, service_name: str):
    ...
    paths_to_delete = [
        row["path"]
        for row in all_rows
        if isinstance(row.get("metadata"), dict) and row["metadata"].get("service") == service_name
    ]
