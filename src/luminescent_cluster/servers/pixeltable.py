...
class PixeltableMemoryServer:
    ...
    async def search_knowledge(
        ...
        if service_filter:
            results = results.where(self.kb.metadata["service"] == service_filter)
        ...
        track_usage(
            "search",
            len(query),
            {"tenant_id": context.get("x-tenant-id"), "result_count": len(result_list)},
        )
    ...
    async def get_adrs(
        self, topic: Optional[str] = None, service: Optional[str] = None, limit: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        results = adrs.select(
            self.kb.path, self.kb.title, self.kb.summary, self.kb.created_at, self.kb.metadata
        ).limit(limit)

        return [dict(row) for row in results.collect()]

    async def get_incidents(
        self, service: Optional[str] = None, limit: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        if service:
            incidents = incidents.where(self.kb.metadata["service"] == service)

        results = (
            incidents.order_by(self.kb.created_at, asc=False)
            .select(self.kb.title, self.kb.summary, self.kb.created_at, self.kb.metadata)
            .limit(limit)
        )
    ...
    async def get_full_content(self, path: str) -> Optional[str]:
        ...
        result = self.kb.where(self.kb.path == path).select(self.kb.content).limit(1)

        items = list(result.collect())
        return items[0]["content"] if items else None

    async def search_meetings(self, query: str, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        results = (
            self.meetings.order_by(sim, asc=False)
            .select(
                self.meetings.title,
                self.meetings.date,
    ...
    async def get_service_context(self, service: str) -> Dict[str, Any]:
        ...
        recent_incidents = list(
            self.kb.where((self.kb.metadata["service"] == service) & (self.kb.type == "incident"))
    ...
    async def ingest_codebase_data(
        ...
        context = context or {}
        logger.debug(
            f"ingest_codebase called: repo_path={repo_path}, service={service_name}, extensions={extensions}"
        )
        ...
        try:
            ...
            track_usage(
                "ingest",
                count,
                {"tenant_id": tenant_id, "service": service_name, "duration_seconds": duration},
            )
    ...
    async def ingest_adr_data(
        self, adr_path: str, title: str, service: str = None
    ) -> Dict[str, Any]:
    ...
    async def ingest_incident_data(self, incident_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def delete_service(
        self, service_name: str, confirm: bool = False, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def prune_old(self, days_old: int, confirm: bool = False) -> Dict[str, Any]:
        ...
        try:
            if name == "search_organizational_memory":
                ...
                result = await memory.get_incidents(
                    service=arguments.get("service"), limit=arguments.get("limit", 5)
                )

            elif name == "get_full_document":
                result = await memory.get_full_content(path=arguments["path"])

            elif name == "search_meeting_transcripts":
                result = await memory.search_meetings(
                    query=arguments["query"], limit=arguments.get("limit", 3)
                )

            elif name == "get_service_overview":
                result = await memory.get_service_context(service=arguments["service"])

            # Write Operations
            elif name == "ingest_codebase":
                result = await memory.ingest_codebase_data(
                    repo_path=arguments["repo_path"], service_name=arguments["service_name"]
                )
                ...
                result = await memory.ingest_incident_data(incident_data=arguments)
            ...
            # Data Management
                ...
                result = await memory.delete_service(
                    service_name=arguments["service_name"], confirm=arguments.get("confirm", False)
                )

            elif name == "prune_old_data":
                result = await memory.prune_old(
                    days_old=arguments["days_old"], confirm=arguments.get("confirm", False)
                )
